...
export const enviarMensagem = (mensagem, contatoNome, contatoEmail) => {
    ...
    return dispatch => {
        ...
        firebase.database().ref(`/mensagens/${usuarioEmailB64}/${contatoEmailB64}`)
            ...
            .then(() => { //Armazenar cabeçalho de conversa do usuário autenticado
                firebase.database().ref(`/usuario_conversas/${usuarioEmailB64}/${contatoEmailB64}`)
                    .set({nome: contatoNome, email: contatoEmail})
            })
            .then(() => { //Armazenar cabeçalho de conversa do contato

                firebase.database().ref(`/contatos/${usuarioEmailB64}`)
                    .once("value")
                    .then(snapshot => {

                        const dadosUsuarios = _.first(_.values(snapshot.val()));

                        firebase.database().ref(`/usuario_conversas/${contatoEmailB64}/${usuarioEmailB64}`)
                            .set({nome: dadosUsuarios.nome, email: usuarioEmail})
                    })
            })        
    }
}
